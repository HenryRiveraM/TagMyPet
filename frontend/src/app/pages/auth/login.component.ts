      ...
      <div class="hidden bg-[url('https://images.unsplash.com/photo-1583337130417-3346a1be7dee?auto=format&fit=crop&w=1100&q=80')] bg-cover bg-center md:block">
        <div class="flex h-full min-h-[560px] items-end bg-slate-950/35 p-8">
          <div class="text-white">
            <p class="eyebrow text-emerald-200">TagMyPet</p>
            <h2 class="mt-3 text-4xl font-bold leading-tight">Cuidado digital para cada mascota.</h2>
          </div>
        </div>
      </div>
      <div class="p-6 sm:p-10">
        <img class="mb-8 h-14 w-14 rounded-lg border border-slate-200 bg-white object-contain p-1 shadow-sm" src="/assets/tagmypet-logo.svg" alt="TagMyPet logo">
        ...
        <form class="mt-8 space-y-4" [formGroup]="form" (ngSubmit)="submit()">
          ...
          <label class="block">
            ...
            <div class="relative">
              ...
              <button type="button" class="absolute right-2 top-1/2 -translate-y-1/2 rounded-md px-3 py-1.5 text-xs font-bold text-brand transition hover:bg-emerald-50" (click)="showPassword.set(!showPassword())">
        ...
        <div class="mt-6 flex flex-wrap gap-x-5 gap-y-2 text-sm font-semibold">
          <a routerLink="/register" class="text-brand hover:text-emerald-700">Crear una cuenta</a>
          <a routerLink="/forgot-password" class="text-brand hover:text-emerald-700">Olvidé mi password</a>
        </div>
      </div>
    </section>
  `
